		...
		if(Char_Size ==16)
		{
	     ...
	     else if(Char_Size ==12)
	     {
			...
			for(i=0;i<12;i++)
			    {
				  if(Color_Turn)
					  OLED_WR_DATA(~ascii_24x12[c][i+12]);
				  else
					  OLED_WR_DATA(ascii_24x12[c][i+12]);
			    }
					OLED_Set_Pos(x,y+2);
			for(i=0;i<12;i++)
			    {
				  if(Color_Turn)
					  OLED_WR_DATA(~ascii_24x12[c][i+24]);
				  else
					  OLED_WR_DATA(ascii_24x12[c][i+24]);
			    }
			 }
...
void OLED_ShowString(uint8_t x,uint8_t y,char*chr,uint8_t Char_Size, uint8_t Color_Turn)
{
	...
	{		OLED_ShowChar(x,y,chr[j],Char_Size, Color_Turn);
			if (Char_Size == 12) //6X8的字体列加6，显示下一个字符
				x += 6;
			else if(Char_Size == 16)  //8X16的字体列加8，显示下一个字符
				x += 8;
			else 
				x+=12;
